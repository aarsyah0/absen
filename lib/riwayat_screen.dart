...
class _RiwayatScreenState extends State<RiwayatScreen> with SingleTickerProviderStateMixin {
  ...
  Future<List<Map<String, dynamic>>> fetchAbsensi() async {
    String urlStr = 'http://localhost:8000/api/employee/attendance/history?length=20&start=0';
    if (_startDate != null) {
  ...
  Future<List<Map<String, dynamic>>> fetchPermission() async {
    String urlStr = 'http://localhost:8000/api/employee/permission/history?length=20&start=0';
    if (_startDate != null) {
      urlStr += '&start_date=${DateFormat('yyyy-MM-dd').format(_startDate!)}';
    }
    if (_endDate != null) {
      ...
      child: Row(
        children: [
          Expanded(
            child: InkWell(
              ...
              child: Container(
                ...
                child: Row(
                  children: [
                    const Icon(Icons.date_range, size: 18, color: Color(0xFF4F8DFD)),
          ...
          ElevatedButton(
            ...
            child: const Text('Terapkan', style: TextStyle(fontSize: 13, color: Colors.white)),
          ),
              ...
              children: [
                ...
                FutureBuilder<List<Map<String, dynamic>>>(
                  ...
                  builder: (context, snapshot) {
                    ...
                    if (snapshot.hasError) {
                      return const Center(child: Text('Gagal memuat data absensi'));
                    }
                    ...
                    return ListView.builder(
                      ...
                      itemBuilder: (context, i) {
                        ...
                        return Card(
                          ...
                          child: ListTile(
                            leading: Icon(item['type'] == 'in' ? Icons.login : Icons.logout, color: const Color(0xFF4F8DFD)),
                            title: Text(item['type'] == 'in' ? 'Check-in' : 'Check-out', style: const TextStyle(fontWeight: FontWeight.bold)),
                            subtitle: Text('${item['date']} ${item['time']}'),
                            trailing: Container(
                              padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
                              decoration: BoxDecoration(
                                color: item['status'] == 'accepted'
                                    ? const Color(0xFF1BCFB4).withOpacity(0.15)
                                    : (item['status'] == 'pending'
                                        ? const Color(0xFFFFC542).withOpacity(0.15)
                ...
                FutureBuilder<List<Map<String, dynamic>>>(
                  ...
                  builder: (context, snapshot) {
                    ...
                    if (snapshot.hasError) {
                      return const Center(child: Text('Gagal memuat data izin'));
                    }
                    final data = snapshot.data ?? [];
                    if (data.isEmpty) {
                    ...
                    return ListView.builder(
                      ...
                      itemBuilder: (context, i) {
                        ...
                        return Card(
                          ...
                          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
                          child: ListTile(
                            leading: const Icon(Icons.assignment_turned_in_outlined, color: Color(0xFF4F8DFD)),
                            title: Text(item['category'] ?? '-', style: const TextStyle(fontWeight: FontWeight.bold)),
                            subtitle: Text('${item['start_date']} - ${item['end_date']}'),
                            trailing: Container(
                              padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
                              decoration: BoxDecoration(
                                color: item['status'] == 'accepted'
        ...
        onTap: (index) {
          ...
          if (index == 0) {
            Navigator.pushAndRemoveUntil(
              context,
              MaterialPageRoute(
                builder: (context) => DashboardScreen(userName: '', token: widget.token),
              ),
              (route) => false,
            );
          }
